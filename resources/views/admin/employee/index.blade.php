...
.btn{padding:12px 20px;border-radius:6px;border:0;background:#111;color:white;cursor:pointer;min-height:44px}
...
.action-btn{padding:6px 10px;border-radius:6px;border:0;cursor:pointer;font-size:13px;min-height:36px}
...
.eye-btn{background:transparent;border:0;cursor:pointer;font-size:16px;margin-left:4px;min-height:36px}

/* Mobile employee cards */
.employee-card{display:none;background:white;border-radius:12px;padding:16px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.05)}
.employee-card-header{display:flex;justify-content:space-between;align-items:start;margin-bottom:12px}
.employee-card-info{flex:1}
.employee-card-name{font-size:18px;font-weight:700;margin:0 0 4px}
.employee-card-detail{font-size:14px;color:#6b7280;margin:4px 0}
.employee-card-actions{display:flex;gap:8px;margin-top:12px}
...
@media (max-width:768px){
  ...
  .topbar form{flex-direction:column;gap:10px}
  .topbar form input, .topbar form select, .topbar form .btn{width:100%;font-size:16px;min-height:44px}
  ...
  .action-btn{width:100%;padding:10px;font-size:16px;min-height:44px}
  ...
  #modalOverlay > div > div[style*="display:flex"] > div{
    width:100% !important;
    flex:1 !important;
  }
  #modalOverlay form > div[style*="display:flex"]{
    flex-direction:column !important;
  }
  #modalOverlay form > div[style*="display:flex"] > div{
    width:100% !important;
    flex:1 !important;
  }
  .input{min-height:44px;font-size:16px;padding:12px}
}
</style>
    ...
    <div class="employee-card">
      <div class="employee-card-header">
        <div class="employee-card-info">
          <h3 class="employee-card-name">{{ $worker->user->name }}</h3>
          <div class="employee-card-detail"><strong>Trade:</strong> {{ $worker->trade }}</div>
          <div class="employee-card-detail"><strong>Username:</strong> {{ $worker->user->username }}</div>
          <div class="employee-card-detail"><strong>Email:</strong> {{ $worker->email ?? 'N/A' }}</div>
          <div class="employee-card-detail"><strong>Phone:</strong> {{ $worker->phone ?? 'N/A' }}</div>
          <div class="employee-card-detail">
            <strong>Password:</strong>
            <span class="pwd-text-mobile" data-hidden="true" data-password="{{ $worker->user->plain_password ?? 'Not set' }}">••••••••</span>
            <button class="eye-btn" onclick="togglePasswordMobile(this)" title="Toggle password visibility" style="display:inline;padding:2px 6px">
              <span class="pwd-icon-mobile">👁️</span>
            </button>
          </div>
        </div>
      </div>
      <div class="employee-card-actions">
        <a href="{{ route('admin.employees.performance', $worker) }}" class="action-btn" style="background:#10b981;color:white;text-decoration:none;text-align:center;flex:1">Performance</a>
        <button class="action-btn edit-btn" onclick='openEditModal(@json($worker))' style="flex:1">Edit</button>
        <form action="{{ route('admin.employees.destroy', $worker) }}" method="POST" style="flex:1" onsubmit="return confirm('Delete this employee?')">
          @csrf
          @method('DELETE')
          <button type="submit" class="action-btn del-btn" style="width:100%">Delete</button>
        </form>
      </div>
    </div>
    @empty
    <div class="employee-card">
      <p style="text-align:center;color:#999;margin:0">No employees found</p>
    </div>
    @endforelse
  </section>
...
function togglePasswordMobile(btn) {
  const pwdText = btn.previousElementSibling;
  const pwdIcon = btn.querySelector('.pwd-icon-mobile');
  const isHidden = pwdText.getAttribute('data-hidden') === 'true';
  const actualPassword = pwdText.getAttribute('data-password');

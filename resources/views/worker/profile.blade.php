...
.btn{background:#7c3aed;color:#fff;border:0;padding:10px 20px;border-radius:8px;cursor:pointer;font-weight:600;transition:all 0.2s;text-decoration:none;display:inline-block}
...
@media (max-width:768px){
  ...
  .avatar-section{width:100%;margin-bottom:16px}
  .bigAvatar{width:140px;height:140px;font-size:48px}
  .input, select.input, textarea.input, .btn{min-height:44px;font-size:16px;padding:12px 14px}
  .btn-small{min-height:44px;padding:10px 16px;font-size:16px;width:100%}
...
<main class="main">
  <div class="container">
    ...
    <div class="section">
      ...
      <p>Update your worker profile information</p>

      <form action="{{ route('worker.profile.update') }}" method="POST" id="profileForm">
        @csrf
        @method('PATCH')
        <input type="hidden" name="avatar" id="avatarData" value="{{ $worker->avatar ?? '' }}">

        <div class="row">
          <div class="avatar-section">
            @if($worker->avatar)
              <div id="avatarPreview" class="bigAvatar" style="background-image:url({{ $worker->avatar }})"></div>
            @else
              <div id="avatarPreview" class="bigAvatar">{{ substr(auth()->user()->name, 0, 1) }}</div>
            @endif
            <input id="avatarInput" type="file" accept="image/*" capture="user" style="display:none" />
          ...
          <div class="col">
            ...
            <input id="name" name="name" type="text" class="input @error('name') error-input @enderror" value="{{ old('name', auth()->user()->name) }}" required />
            @error('name')
            <div class="error-msg">{{ $message }}</div>
            @enderror

            <label for="email">Email</label>
            <input id="email" name="email" type="email" class="input @error('email') error-input @enderror" value="{{ old('email', $worker->email) }}" />
            @error('email')
            <div class="error-msg">{{ $message }}</div>
            @enderror

            <label for="phone">Phone</label>
            <input id="phone" name="phone" type="text" class="input @error('phone') error-input @enderror" value="{{ old('phone', $worker->phone) }}" />
            @error('phone')
            <div class="error-msg">{{ $message }}</div>
            @enderror
            ...
            <div class="row" style="margin-top:0">
              <div class="col">
                ...
                <select id="sex" name="sex" class="input">
                  ...
                  <option value="Male" {{ old('sex', $worker->sex) == 'Male' ? 'selected' : '' }}>Male</option>
                  <option value="Female" {{ old('sex', $worker->sex) == 'Female' ? 'selected' : '' }}>Female</option>
                  <option value="Other" {{ old('sex', $worker->sex) == 'Other' ? 'selected' : '' }}>Other</option>
                </select>
              </div>

              <div class="col">
                <label for="dob">Date of Birth</label>
                <input id="dob" name="dob" type="date" class="input" value="{{ old('dob', $worker->dob) }}" />
              </div>
            </div>

            <label for="notes">Notes</label>
            <textarea id="notes" name="notes" class="input" rows="3">{{ old('notes', $worker->notes) }}</textarea>

            <div style="margin-top:20px;display:flex;gap:8px">
              <button type="submit" class="btn">Save Changes</button>
              <a href="{{ route('worker.dashboard') }}" class="btn btn-secondary">Cancel</a>
            </div>
          </div>
        </div>
      </form>
    ...
    <div class="section">
      ...
      <form action="{{ route('worker.profile.update-password') }}" method="POST">
        @csrf
        @method('PATCH')
    ...
    <div class="section">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px">
        <div>
          ...
          <div style="font-weight:600;margin-top:4px">{{ ucfirst(auth()->user()->role) }}</div>
        </div>
        <div>
          <div style="font-size:13px;color:#6b7280">Trade</div>
          <div style="font-weight:600;margin-top:4px">{{ $worker->trade }}</div>
        </div>
        <div>
          <div style="font-size:13px;color:#6b7280">Tasks Assigned</div>
          <div style="font-weight:600;margin-top:4px">{{ $worker->tasks_assigned }}</div>
        </div>
        <div>
          <div style="font-size:13px;color:#6b7280">Account Created</div>
          <div style="font-weight:600;margin-top:4px">{{ auth()->user()->created_at->format('M d, Y') }}</div>
        </div>
      </div>
    </div>
  </div>
